function [Params,Files,NS6] = PrePro_DS( ParamFile )
...
% second bank.
[NS6] = HeaderHandling(datapath,Files);
...
% Nested Functions
    ...
    function [NS6] = HeaderHandling(datapath,Files)
        ...
        for filenumber = 1:length(Files.ns6)
            ...
            if Chops.all
                ...
                for ep = 1:length(EpochStarts)
                    ...
                    Epoch = downsample(Epoch',30)';
                    
                    % Write to a new file ('filenumber01.6ep001_bank1')
                    for Bank = 1:NumBanks
                        [SplitEpochs{Bank}] = Epoch(BankChannels{Bank},:);
                        SplitEpochs{Bank} = SplitEpochs{Bank}(:);
                        newfname6 = [datapath '\lfp\filenumber',num2str(filenumber,'%02.0f'),'.6ep',num2str(ep,'%03.0f'),'_bank',num2str(Bank)];
                ...
                for S = 1:size(Params.splitwindows,1)
                    ...
                    for ep = 1:length(EpochStarts)
                        ...
                        for Bank = 1:NumBanks
                            [SplitEpochs{Bank}] = Epoch(BankChannels{Bank},:);
                            SplitEpochs{Bank} = SplitEpochs{Bank}(:);
                            newfname6 = [datapath '\lfp\split',num2str(S,'%02.0f'),'filenumber',num2str(filenumber,'%02.0f'),'.6ep',num2str(ep,'%03.0f'),'_bank',num2str(Bank)];
                            FIDw = fopen(newfname6, 'w+', 'ieee-le');
                            fwrite(FIDw, SplitEpochs{Bank}, 'int16');
                            fclose(FIDw);
                        end
