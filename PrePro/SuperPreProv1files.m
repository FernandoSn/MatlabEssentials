function [Params,Files,NS6] = SuperPreProv1files(ParamFile, DatON, RefON, DSON)
...
%   FKA PrePro_SC_edit2
%
% DatON says whether to make a dat file, RefON says whether to do
% referencing, DS says whether to do downsampling
%
...
%% Concatenate downsampled NS6 epochs
if DSON
    if Chops.all
        [~, fname] = fileparts(Files.ns6{1});
        for Bank = 1:NumBanks
            %         finalfilename = [datapath,filesep,'epochs',filesep,fname,'_bank',num2str(Bank),'.dat'];
            finalfilename = [datapath,filesep,'lfp',filesep,fname,'_bank',num2str(Bank),'.lfp'];
            D = dir([datapath,'\lfp\*6ep*bank',num2str(Bank)]);
            [CatList,~] = sort({D.name}); % output of dir command is not guaranteed to be sorted but concatenation has to happen in order
            for ep = 1:length(CatList)
                if ep == 1
                    CatCmd6 = ['copy /b ' [datapath,filesep,'lfp',filesep,CatList{ep}] ,' ',finalfilename];
                    system(CatCmd6);
                else
                    CatCmd6 = ['copy /b ' finalfilename '+' [datapath,filesep,'lfp',filesep,CatList{ep}], ' ', finalfilename];
                    system(CatCmd6);
                end
            end
        end
        system(['del ',datapath,'\lfp\*6ep*'])
    elseif Chops.split
        [~, fname] = fileparts(Files.ns6{1});
        for Bank = 1:NumBanks
            for S = 1:size(Params.splitwindows,1)
                finalfilename = [datapath,filesep,'lfp',filesep,fname,'split',num2str(S,'%02.0f'),'_bank',num2str(Bank),'.dat'];
                D = dir([datapath,'\lfp\split',num2str(S,'%02.0f'),'*6ep*bank',num2str(Bank)]);
                [CatList,~] = sort({D.name}); % output of dir command is not guaranteed to be sorted but concatenation has to happen in order
                for ep = 1:length(CatList)
                    if ep == 1
                        CatCmd6 = ['copy /b ' [datapath,filesep,'lfp',filesep,CatList{ep}] ,' ',finalfilename];
...
% Nested Functions
    ...
    function [NS6,NS3] = HeaderHandling(datapath,Files)
        ...
        BasicHeader = fread(FID, 72, 'uint32');
        NS3.ChannelCount = double(BasicHeader(8));
        
        if NS3.ChannelCount == 8;
            NS3.HeaderBytes = 64;
        ...
        BasicHeader = fread(FID, 72, 'uint32');
        NS6.ChannelCount = double(BasicHeader(8));
        
        if NS6.ChannelCount == 64;
            NS6.HeaderBytes = 288;
            NS6.Bloc = 33;
            
        elseif NS6.ChannelCount == 32;
            NS6.HeaderBytes = 160;
            NS6.Bloc = [];
            
        end
        
%         a = fread(FID, NS6.HeaderBytes, 'uint8');
        
        
        
%                 
%         
%         Recording = fread(FID, 64*30000*2*5, 'int16');
%         RecTotal = Recording;
% %         Recording = Recording(257:end);
%         Recording = Recording(length(Recording) - 30000*64 + 1-(64*302):end);
% 
%         RecMat = zeros(64,(length(Recording)/64));
%             for ii = 1:length(Recording)
% 
%                 RecMat(ii) = Recording(ii);
% 
%             end
%             
%         y = abs(fft(RecMat(1,:)- mean(RecMat(1,:)) ));
%         freq = 0:(30000/2)/(length(y)/2):30000/2;
%         
%         fseek(FID, 288 - 64*2, 'bof');
%         Recording2 = fread(FID, 64*30000*2*5, 'int16');
%         
%          RecMat2 = zeros(64,(length(Recording2)/64));
%             for ii = 1:length(Recording2)
% 
%                 RecMat2(ii) = Recording2(ii);
% 
%             end
    ...
    function [NumBanks] = NS6EpochWriting(datapath,Files,NS6,Params)
        ...
        for filenumber = 1:length(Files.ns6)
            ...
            % Read Size
            NS6.Fs = 30000; % Sampling frequency for NS6 files
            EpochSeconds = 300;
            % EpochSamples is not in Bytes. To turn this into Bytes multiply by 2.
            ...
            if Chops.all
                ...
                for ep = 1:length(EpochStarts)
                    ...
                    Epoch_DS = downsample(Epoch',30)';
                    
                    % Write to a new file ('filenumber01.6ep001_bank1')
                    for Bank = 1:NumBanks
                        if DatON
                            if RefON
                                [SplitEpochs{Bank}] = Referee(Epoch(BankChannels{Bank},:));
                            else
                        ...
                        if DSON
                            [SplitEpochs_DS{Bank}] = Epoch_DS(BankChannels{Bank},:);
                            SplitEpochs_DS{Bank} = SplitEpochs_DS{Bank}(:);
                            newfname6_DS = [datapath '\lfp\filenumber',num2str(filenumber,'%02.0f'),'.6ep',num2str(ep,'%03.0f'),'_bank',num2str(Bank)];
                            FIDw = fopen(newfname6_DS, 'w+', 'ieee-le');
                            fwrite(FIDw, SplitEpochs_DS{Bank}, 'int16');
                            fclose(FIDw);
                        end
                    end
                    ...
                    for ep = 1:length(EpochStarts)
                        ...
                        for Bank = 1:NumBanks
                            ...
                            if DSON
                                [SplitEpochs_DS{Bank}] = Epoch_DS(BankChannels{Bank},:);
                                SplitEpochs_DS{Bank} = SplitEpochs_DS{Bank}(:);
                                newfname6_DS = [datapath '\lfp\split',num2str(S,'%02.0f'),'filenumber',num2str(filenumber,'%02.0f'),'.6ep',num2str(ep,'%03.0f'),'_bank',num2str(Bank)];
                                FIDw = fopen(newfname6_DS, 'w+', 'ieee-le');
                                fwrite(FIDw, SplitEpochs_DS{Bank}, 'int16');
                                fclose(FIDw);
                            end
...
% Doubly-Nested Function
    function [ReferencedData] = Referee(NeuralData)
        Fs = 30000;
        NeuralData = NeuralData'; % Now channels are columns because filters work down columns;
        
        %% cut line noise
        [B,A] = butter(2,[59/(Fs/2) 61/(Fs/2)],'stop');
        dataline = filtfilt(B,A,double(NeuralData));
        
        %% high pass filter the data
        low = 100;
        [D,C] = butter(2,low/(Fs/2),'high');
        data = filtfilt(D,C,dataline);
        % channels are still columns
        
        %% Exclude weird or dead channels from consideration as references.
        % channels whose variance is different from the average variance by 2.5 SD
        % are bad
        a = std(data);
        a = a';
        ...
        % and subtract it from the original high-pass filtered data
        datarefd = bsxfun(@minus,data,refch);
        
        %% linearize and convert back to int16
        ReferencedData = int16(datarefd(:));
    end
end
