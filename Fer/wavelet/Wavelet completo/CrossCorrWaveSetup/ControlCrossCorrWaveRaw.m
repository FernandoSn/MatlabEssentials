function FullWCrossCorrRaw = ControlCrossCorrWaveRaw(name1,name2)
archivos=ls('*.mat');


n = 4;

FullWCrossCorrRaw = cell(size(archivos,1),2);

for noArchivo = 1:size(archivos,1)
    
    FullWCrossCorrRaw{noArchivo,1} = archivos(noArchivo,:);
    ...
    for channel = 1:3
        ...
        [WCrossCorrA, WCrossCorrR, freq, ~]...
            = waveletCrossCorrelationRaw(datos.registro(:,channel1),datos.registro(:,channel2),1000,1/12);
    ...
    FullWCrossCorrRaw{noArchivo,2} = CrossStructStack;
    
end

        fprintf ('Saving the full Wavelet Cross Correlation into a .mat...\n');
        
        save('FullWCrossCorrRaw.mat','FullWCrossCorrRaw');
