function [deltaCoH,deltaCross,tetaCoH,tetaCross,...
    ...
    deltaCrossVar,tetaCrossVar,alphaCrossVar,betaCrossVar,gammaCrossVar] = controlCoHSWD(registro,channel1,channel2)        

%Inidicaciones, ajustar la variazna dependiendo si se normaliza con
%transformada compleja o espectro de potencia.
%Solo para experimento AbsNPS.
...
[coH,crossSpectrum,freq,~,crossSpectrumVar] = waveletCoherence(reg1,reg2,2000,1/12);

coH = coH(:,8000:9999);
crossSpectrum = crossSpectrum(:,8000:9999);
%crossSpectrumVar = crossSpectrumVar(:,8000:9999);

%Se corta el espectro a un segundo.
