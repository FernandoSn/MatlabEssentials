...
function Scores = NPX_SCOmakerPreInh(NPXMCSR,NPXMCSRPreInh)
...
if length(NPXMCSR{1}) ~= length(NPXMCSRPreInh{1})
    error('Both Cell arrays must have the same trials, units and valves');
end
    ...
    if ~isempty(TrialSets{tset})
        for Unit = 1:size(NPXMCSR,2)
            for Valve = 1:size(NPXMCSR,1)   
                ...
                [Scores.auROC(Valve,Unit,tset), Scores.AURp(Valve,Unit,tset)] = RankSumROC(NPXMCSRPreInh{Valve,Unit}(TrialSets{tset}),NPXMCSR{Valve,Unit}(TrialSets{tset}));

                % Z-Scores based on valve 1 responses vs everything else.
                Scores.ZScore(Valve,Unit,tset) = (nanmean(NPXMCSR{Valve,Unit}(TrialSets{tset}))-nanmean(NPXMCSRPreInh{Valve,Unit}(TrialSets{tset})))./nanstd(NPXMCSRPreInh{Valve,Unit}(TrialSets{tset}));
                ...
                Scores.RateChange(Valve,Unit,tset) = (nanmean(NPXMCSR{Valve,Unit}(TrialSets{tset}))-nanmean(NPXMCSRPreInh{Valve,Unit}(TrialSets{tset})));
