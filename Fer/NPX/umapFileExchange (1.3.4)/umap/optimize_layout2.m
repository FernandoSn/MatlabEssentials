...
%
    ...
    if TEST_CROSS_ENTROPY
            ...
            disp(['The initial cross entropy is ' num2str(CE)]);
            disp(['The initial approximated cross entropy is ' num2str(approx_CE)]);
    end
    for n = 1:n_epochs
        ...
        if verbose && mod(n, progress_checkpoint) == 0
            ...
            if TEST_CROSS_ENTROPY
                ...
                %CE = cross_entropy(dists, weights, min_dist);
                approx_CE = approx_cross_entropy(dists, weights, a, b);
                result = full_approx_cross_entropy(double(head_embedding), double(tail_embedding), double(head), double(tail), double(weights), a, b, same_embedding);
