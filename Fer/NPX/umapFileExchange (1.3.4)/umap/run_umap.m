...
%                           If verbose=graphic then the data displays with
%                           probability coloring and contours as is
%                           conventional in flow cytometry analysis. If
%                           method=Java then the display refreshes as
%                           optimize_layout progresses and a progress bar
%                           is shown along with a handy cancel button.
...
%                           environment.  A faster implementation can be
%                           had by developing a custom C module for
%                           optimize_layout.  The current C programming
...
%                           Default is 0.6.
%
%   'save_template_file'    Fully qualified path of the file to save the umap 
%                           as a template (alternate to output parameter).
%
...
%                           relatedness of data groupings in the supervising 
%                           and supervised embeddings. The above 
%                           documentation for the match_supervisors
%                           argument defines "data groupings".
%                           The publication that introduces the QF tree is
...
%                           template.
%                           
%   joined_transform        Use Connor's NEW transform method to avoid
%                           false positives when applying a  template
%                           whose training data differs too much from test
%                           test set data.
%                           This feature is not part of UMAP's original 
...
if strcmpi(method, 'Java')
    if plotting
        ...
        try
            ...
            figure(fig);
            if args.qf_dissimilarity && args.qf_tree
                puLocation='north+';
            else
                puLocation='south++';
            end
...
if nargout>1 || ~isempty(save_template_file)
                ...
                if hasLabels
                    if nargin>1 && lastCall
                        ...
                        if args.qf_tree
                            ...
                            umap.supervisors.qfTreeSupervisors;
                        end
                    else
                        ProbabilityDensity2.DrawLabeled(curAxes, data, ...
                            labels, labelMap);
                    ...
                    if isprop(umap, 'supervisors') ...
                        ...
                        if nargin>1 && lastCall
                            ...
                            umap.supervisors.computeAndMatchClusters(data, 0);
                            [lbls, lblMap]=...
    ...
    function p=parseArguments(varargin)
        ...
        expectedMetric = {'euclidean', 'cosine', 'cityblock', 'seuclidean',...
            'squaredeuclidean', 'correlation', 'jaccard', 'spearman', ...
            'hamming', 'mahalanobis'};
