function [InhTimes,PREX,POSTX,RRR,BbyB] = BoldingResp(RESP,Fs,t,FVOpens,FVCloses)

%RESP = RESP - mean(RESP); %remove DC
%%Bolding code
...
function [RRR,sgdtresp,Rrms,flt] = FilterAndFlatten(asRESP,Fs,t)
    ...
    % (little peaks)
    sgresp = sgolayfilt(asRESP,5,(0.185*Fs)+1); %Fs = 2000; Bolding param
    
    %sgresp = sgolayfilt(asRESP,5,(0.15*Fs)+1); %Fs = 2000;
    
    %%sgresp = sgolayfilt(asRESP,5,(0.186*Fs));% Fs = 2500;
    %sgresp = sgolayfilt(asRESP,5,1071);% Fs = 2500;
