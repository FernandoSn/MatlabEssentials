function [SpikeTimes] = SpikesSamplesProFer(KWIKfile, SpikeType)
...
%Fer function returns SpikeSamples not Times. If you want to convert to
%times just dive between the sample rate which is 30000. this is helpful to
%do analysis in C++ programs.
...
% Comment this beacuse I want to create the spike times every time for now.
...
if ~isempty(strfind(FilesKK.KWIK,'result'))
    ...
    for unit = 1:length(b)
        ...
        tsecs{unit+1} = h5read(FilesKK.KWIK,['/spiketimes/',b{unit}]);
        tsecs{unit+1} = round(double(tsecs{unit+1}(:)));
    end
