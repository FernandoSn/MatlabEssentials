...
[~,BLANKVAR,~] = VSMultiCycleCount(efd.ValveTimes,SpikeTimes,efd.PREX,{-1});
        ...
        if ~isempty(TrialSets{tset})
            for Unit = 1:size(TESTVAR,2)
                for Cycle = 1:size(TESTVAR,3)
                    % Blank Rate and SD
%                     Scores.BlankRate(Unit,Cycle,tset) = nanmean(BLANKVAR{Valve,Unit,1}(TrialSets{tset}));
%                     Scores.BlankSD(Unit,Cycle,tset) = nanstd(BLANKVAR{Valve,Unit,1}(TrialSets{tset}));
