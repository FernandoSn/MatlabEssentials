function [WaveForms, Amps, ST] = WaveFormsPro_Window(FilesKK, SpikeType, Window)
...
if ~isempty(strfind(FilesKK.KWIK,'result'))
    ...
    for unit = 1:length(SpikeTimes.units)-1
        ...
        subspks = SpikeTimes.tsec{unit+1}(SpikeTimes.tsec{unit+1}>Window(1) & SpikeTimes.tsec{unit+1}<Window(2));
        subspks = subspks(randperm(length(subspks),min(wfsubsample,length(subspks))));
        ...
        WaveForms{unit+1} = WFx;
        Amps{unit+1} = peak2peak(mean(WFx,3)');
        ST{unit+1} = subspks;
    end
    %
...
switch SpikeType
    ...
    case 'Good'
        ...
        for unit = 1:length(GoodClusters)
            ...
            subspks = TSECS{unit+1}(TSECS{unit+1}>Window(1) & TSECS{unit+1}<Window(2));
            subspks = subspks(randperm(length(subspks),min(wfsubsample,length(subspks))));
