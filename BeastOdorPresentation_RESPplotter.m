...
%% Create trial params
% Miscellany
nseries = 10; % Number of times each odor is presented in a block
nloops = 1;

% Timing
params.TrialLength = 8; % Including presentation time.
params.openTime = .5; % Length of time odor will be presented.

% Panel and Valence
% MFC1 odor pin options: 24 26 28 29 30 31 32 33
% MFC2 odor pin options: 40 41 42 43 44 45 47 49
% 31 and 42 need to always be mineral oil or Beast will flip shit.

% params.odorchannels = [24 26 28:1:33 40:1:45 47 49];
% params.odorchannels = [24 26 28 30];
% params.odorchannels = [26,28,29,30,31,32,33,40,41,43,47,49];
% params.odorchannels = [40 41 42 43 44 45 47 49];
params.odorchannels = [41 43];

params.odorconc = [1];
% params.odorconc = 10.^-([.5:-.25:0]);
...
%             ylim([0 YL])
            ...
            current_max = max(current_data(t > 4 & t < (params.TrialLength-1)));
            
            subplot(2,length(Ir),odor_plot_idx+length(Ir))
            semilogx(DC(i,1),log10(max(current_data)),'k.', 'markersize',20); hold on
%             xlim([-.1 1.1])
            axis square
%              ylim([1 4])
        end
