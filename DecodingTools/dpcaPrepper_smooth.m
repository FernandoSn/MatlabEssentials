function [SpikeMat,PSTHt] = dpcaPrepper_smooth(Catalog, efds, BinSize, PST)
...
for R = 1:length(KWIKfiles)
    ...
    Trials = FT(R):FT(R)+min_trials-1;
        ...
        [~, PSTHtrials, PSTHt] = KDFmaker(efd.ValveSpikes.RasterAlign, PST, BinSize, Trials);
        realPST = PSTHt>PST(1) & PSTHt<PST(2);
        
        for N = 1:size(PSTHtrials,2)
            for S = 1:size(PSTHtrials,1)
                for T = 1:size(PSTHtrials,3)
                    Spikes{R}(N,S,:,T) = PSTHtrials{S,N,T}(realPST);
                end
            end
        end
