...
for R = 1:length(KWIKfiles)
    [bROC_LR{R}, bROCp_LR{R}] = BinROCMaker_Beast(efd(R).ValveSpikes.RasterAlign(1:11,Conc,:), [0 .5], .06);
    [bROC_nLR{R}, bROCp_nLR{R}] = BinROCMaker_Beast(efd(R).ValveSpikes.RasterAlign(1:11,Conc,:), [0 .5], .06);
end
...
for R = 1:length(KWIKfiles)
    for valve = 1:length(VOI)
        for unit = 1:length(LR_idx{R,1})
            if sum(bROCp_LR{R}((valve+1),LR_idx{R,1}(unit),:) < .05 & bROC_LR{R}((valve+1),LR_idx{R,1}(unit),:) > .5) > 0
                LR_enh_idx{R}(valve,unit) = 1;
            else
                LR_enh_idx{R}(valve,unit) = 0;
            end
                
            if sum(bROCp_LR{R}((valve+1),LR_idx{R,1}(unit),:) < .05 & bROC_LR{R}((valve+1),LR_idx{R,1}(unit),:) < .5) > 0
                LR_supp_idx{R}(valve,unit) = 1;
            else
                LR_supp_idx{R}(valve,unit) = 0;
            end
        end
    end
end
...
for R = 1:length(KWIKfiles)
    for valve = 1:length(VOI)
        for unit = 1:length(LR_idx{R,2})
            if sum(bROCp_nLR{R}((valve+1),LR_idx{R,2}(unit),:) < .05 & bROC_nLR{R}((valve+1),LR_idx{R,2}(unit),:) > .5) > 0
                nLR_enh_idx{R}(valve,unit) = 1;
            else
                nLR_enh_idx{R}(valve,unit) = 0;
            end
                
            if sum(bROCp_nLR{R}((valve+1),LR_idx{R,2}(unit),:) < .05 & bROC_nLR{R}((valve+1),LR_idx{R,2}(unit),:) < .5) > 0
                nLR_supp_idx{R}(valve,unit) = 1;
            else
                nLR_supp_idx{R}(valve,unit) = 0;
            end
        end
    end
end
